---
title: "How SSB Analysis Works — and Why I Built It"
...
That's the core idea behind SSB Analysis: **make the AI prove every claim by writing the code that produces it.**

## What the pipeline actually does

Every three days, a two-phase AI pipeline runs on GitHub Actions and produces a new post about Norwegian statistics. A weekly Friday edition goes further, forecasting a series with modern time-series machine learning.
...
The site is rendered with Quarto and deployed to GitHub Pages. An automatic error-fixer scans the rendered output for failed code blocks, patches them with Claude, and logs the error pattern so the next run's pipeline knows to avoid it.
...
*The full pipeline source is in `ssb-daily/generate_post.R`, `ssb-daily/generate_special.R` and `ssb-daily/fix_post.R` in the [site repository](https://github.com/simbje/simbje.github.io). Posts are generated every three days and cover a different dataset each time, with a machine-learning forecasting edition every Friday.*
