...
pts <- points_raw |>
  mutate(
    ...
    GameNo        = as.integer(GameNo),
    PointNumber   = as.integer(PointNumber),
    PointServer   = as.integer(PointServer),
    PointWinner   = as.integer(PointWinner),
    GameWinner    = as.integer(GameWinner),
    RallyCount    = as.integer(RallyCount),
    Speed_KMH     = as.numeric(Speed_KMH),
    P1DistanceRun = as.numeric(P1DistanceRun),
    P2DistanceRun = as.numeric(P2DistanceRun),
    across(c(P1Ace, P2Ace, P1Winner, P2Winner,
             P1DoubleFault, P2DoubleFault,
             P1UnfErr, P2UnfErr,
             P1BreakPoint, P2BreakPoint,
             P1BreakPointWon, P2BreakPointWon),
...
# Join points to matches to get match winner
dist_data <- pts |>
  ...
  filter(!is.na(winner)) |>
  mutate(
    winner_dist = if_else(winner == 1, P1DistanceRun, P2DistanceRun),
    loser_dist  = if_else(winner == 1, P2DistanceRun, P1DistanceRun)
  ) |>
  group_by(match_id, slam, year, winner) |>
  summarise(
