...
description: "Bi-weekly scan of recent research in data analytics, data science, finance, and quantitative finance — top papers, topic timeline, and an interactive mind graph."
date: today
date-format: long
...
Every two weeks, a Claude AI agent scans recent papers from **arXiv** (`q-fin.*`, `stat.ML`, `cs.LG`) and **SSRN** (best-effort), clusters them into named topics, and surfaces the three most-trending pieces of work.

The page below reads every snapshot ever produced. The **timeline** shows how topics rise and fall over time. The **mind graph** is built from co-membership — two topics are connected when papers belong to both.
:::
...
# Build a long table of (run_date, topic_name, paper_ids) across all snapshots
...
## This fortnight's top 3

```{r top-3, eval = has_data, results = "asis"}
tops <- latest$top_papers %||% list()
if (length(tops) == 0L) {
  cat("_No top papers in the latest snapshot._\n")
} else {
  for (p in tops) {
...
# Build per-topic aggregated paper sets across all snapshots
...
# Compute edges = paper_id intersection across topic pairs
edges <- if (nrow(topic_papers) >= 2) {
  ...
  map_dfr(pairs, function(pr) {
    ...
    if (w >= 1) tibble(from = pr[1], to = pr[2], weight = w) else tibble()
  })
} else tibble(from = character(), to = character(), weight = integer())

# Communities (Louvain) — only meaningful if we have edges
if (nrow(edges) > 0L) {
...
## Latest run — all top papers

```{r table, eval = has_data}
papers_df <- map_dfr(latest$top_papers %||% list(), function(p) {
  tibble(
    Rank    = p$rank,
    Title   = sprintf('<a href="%s" target="_blank">%s</a>', p$url, p$title),
    Authors = paste(unlist(p$authors %||% list()), collapse = ", "),
    Source  = p$source,
    Topics  = paste(unlist(p$topics %||% list()), collapse = ", "),
    Why     = p$why_trending
  )
})

if (nrow(papers_df) == 0L) {
  htmltools::tags$div(class = "alert alert-secondary",
                      "Latest snapshot has no top-paper picks.")
} else {
  DT::datatable(
    papers_df,
    escape   = FALSE,
...
- **Phase A (deterministic)** — pulls last 14 days from arXiv (`q-fin.*`, `stat.ML`, `cs.LG`); attempts SSRN (Cloudflare-protected, best-effort).
- **Phase B (Claude Haiku tool-use)** — clusters candidates into 5–10 named topics and picks the top 3 most-trending. Edges are *not* emitted by the model — they're computed from `paper_ids` intersections to guarantee consistency.
...
rows <- map_chr(recent, function(s) {
  sprintf("- **%s** — %d candidates, %d topics, sources: %s",
          s$run_date,
          s$candidates_count %||% 0L,
          length(s$topics %||% list()),
          paste(unlist(s$sources_used %||% list()), collapse = ", "))
})
